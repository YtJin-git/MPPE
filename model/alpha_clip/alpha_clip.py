...
def load(name: str, alpha_vision_ckpt_pth="None", device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, context_length=77, lora_adapt=False, rank=16):
    """Load a CLIP model
    ...
    if not jit:
        model = build_model(state_dict or model.state_dict(), context_length=context_length, lora_adapt=lora_adapt, rank=rank).to(device)
        if str(device) == "cpu":
            model.float()
