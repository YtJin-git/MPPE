...
class MPPE(nn.Module):
    def __init__(self, config, attributes, classes, offset):
        ...
        # primitives enhancement module
        self.pe = nn.ModuleList([CrossAttentionLayer(output_dim, output_dim // 64, self.cross_attn_dropout) for _ in
    ...
    def loss_calu(self, predict, target):
        ...
        if len(predict) == 5:
            comp_logits, attr_logits, obj_logits, comp_pe_a2o_logits, comp_pe_o2a_logits = predict
        else:
            comp_logits, attr_logits, obj_logits, comp_pe_a2o_logits = predict
            comp_pe_o2a_logits = comp_pe_a2o_logits
        ...
        loss_comp_pe_a2o = loss_fn(comp_pe_a2o_logits, batch_target)
        loss_comp_pe_o2a = loss_fn(comp_pe_o2a_logits, batch_target)
        loss = loss_comp * self.config.pair_loss_weight + \
               loss_attr * self.config.attr_loss_weight + \
               loss_obj * self.config.obj_loss_weight + \
               loss_comp_pe_a2o * self.config.pair_pe_a2o_loss_weight + \
               loss_comp_pe_o2a * self.config.pair_pe_o2a_loss_weight
        return loss

    def logit_infer(self, predict, pairs):
        if len(predict)==5:
            comp_logits, attr_logits, obj_logits, comp_pe_a2o_logits, comp_pe_o2a_logits = predict
            comp_logits = (comp_logits + comp_pe_a2o_logits * self.config.pair_pe_a2o_inference_weight +
                           comp_pe_o2a_logits * self.config.pair_pe_o2a_inference_weight)
        else:
            comp_logits, attr_logits, obj_logits, comp_pe_a2o_logits = predict
            comp_logits = (comp_logits + comp_pe_a2o_logits * self.config.pair_pe_a2o_inference_weight)
    ...
    def encode_text_for_open(self, idx):
        ...
        for i in range(idx.shape[0]):
            ...
            # a2o
            text_feature_pe = text_feature_o
            for layer in self.pe:
                text_feature_pe = layer(text_feature_pe, text_feature_a)

            text_feature_pe = text_feature_o.squeeze() + self.lamda * text_feature_pe.squeeze()
            text_features_c_a2o.append(text_feature_pe)
    ...
    def forward(self, batch, idx):
        ...
        text_features_c_a2o = list()  # q: text_feature_o，kv: text_feature_a
