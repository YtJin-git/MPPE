...
def predict_logits_text_first(model, dataset, config):
    ...
    with torch.no_grad():
        ...
        for i_text_batch in range(num_text_batch):
            ...
            cur_text_feats = model.module.encode_text_for_open(cur_pair)
        ...
        for idx, data in tqdm(
            ...
            # batch_img = data[0].cuda()
            predict = model.module.forward_for_open(data, text_feats)
            logits = model.module.logit_infer(predict, pairs)
            loss += model.module.loss_calu(predict, data).item()
            attr_truth, obj_truth, pair_truth = data[2], data[3], data[4]
            # attr_truth, obj_truth, pair_truth = data[1], data[2], data[3]
            logits = logits.cpu()
            all_logits = torch.cat([all_logits, logits], dim=0)
...
if __name__ == "__main__":
    ...
    model = get_model(config, attributes=attributes, classes=classes, offset=offset).to('cuda')
    # 遍历所有模型权重
    print(f"============Now weights of epoch_8 is using!=============")
    model.load_state_dict(torch.load(os.path.join(
        "logs_cgqa_ow", "epoch_8.pt"
    )))
